...
class AutoTrader(BaseAutoTrader):
    ...
    def __init__(self, loop: asyncio.AbstractEventLoop, team_name: str, secret: str):
        ...
        self.best_etf_bid_price = self.best_etf_ask_price = 0
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.FUTURE:
            ...
            self.best_etf_ask_price = ask_prices[0] if ask_prices[0] != 0 else 0
            self.best_etf_bid_price = bid_prices[0] if bid_prices[0] != 0 else 0
    ...
    def on_order_filled_message(self, client_order_id: int, price: int, volume: int) -> None:
        ...
        if client_order_id in self.bids:
            ...
            self.send_hedge_order(next(self.order_ids), Side.ASK, (price+100)//TICK_SIZE_IN_CENTS*TICK_SIZE_IN_CENTS, volume)
        elif client_order_id in self.asks:
            self.position -= volume
            self.send_hedge_order(next(self.order_ids), Side.BID, (price-100)//TICK_SIZE_IN_CENTS*TICK_SIZE_IN_CENTS, volume)
